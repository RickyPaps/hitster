...
// Phase transition animation configs for GSAP
const PHASE_ENTER: Record<string, { from: gsap.TweenVars; to: gsap.TweenVars; duration: number }> = {
  SPINNING: {
  ...
  DRINKING_SEGMENT: {
    from: { opacity: 0, scale: 0.85, rotation: -2 },
...
function CenterContent({
  ...
  const prevCenterPhaseRef = useRef(phase);

  // GSAP phase transition animation
  useEffect(() => {
    const el = centerRef.current;
    if (!el) return;
    const prev = prevCenterPhaseRef.current;
    prevCenterPhaseRef.current = phase;
    ...
    const config = PHASE_ENTER[phase] || PHASE_ENTER.SPINNING;
    const announceDelay = showRoundAnnounce && phase === 'PLAYING' ? 1.4 : 0;

    const ctx = gsap.context(() => {
      // Exit old content quickly, then enter new
      const tl = gsap.timeline();
      if (prev !== phase) {
        tl.to(el, { opacity: 0, scale: 0.95, duration: 0.2, ease: 'power2.in' });
      }
      tl.fromTo(el, config.from, {
        ...config.to,
          ...
          <div className="flex flex-col items-center w-full max-w-5xl">
            ...
            <div className="relative w-full">
              ...
              <div
                ...
                style={{ background: 'linear-gradient(135deg, #00f2ff, #ff007f, #bc13fe, #00f2ff)' }}
              >
              ...
              <div
                ...
                style={{
                  ...
                  border: `3px solid ${timerColor}`, boxShadow: timerGlow,
                }}
              >
                <svg width="70" height="70" className="absolute rotate-[-90deg]">
                  ...
                  <circle
                    cx="35" cy="35" r="30" fill="none" stroke={timerColor} strokeWidth="3" strokeLinecap="round"
                    strokeDasharray={2 * Math.PI * 30}
                    strokeDashoffset={2 * Math.PI * 30 * (1 - (maxTimer > 0 ? timerSeconds / maxTimer : 0))}
                    style={{ transition: 'stroke-dashoffset 1s linear, stroke 0.3s' }}
                  />
                </svg>
                <span className={`font-black text-lg tabular-nums ${timerSeconds <= 10 ? 'animate-pulse' : ''}`} style={{ color: timerColor }}>
      ...
      return (
        <div className="flex flex-col items-center gap-8 w-full max-w-4xl">
          <div className="w-full relative group">
            ...
            <div className="relative px-4 py-8 sm:px-8 sm:py-12 challenge-card rounded-2xl flex flex-col items-center text-center">
              ...
              <h1
                ...
                style={{ color: '#00f2ff', letterSpacing: '-0.02em', filter: 'drop-shadow(0 0 15px rgba(0, 242, 255, 0.6))' }}
              >
                Guess the {categoryLabel}!
              </h1>
              ...
              <div className="flex items-center justify-center gap-6 mt-4 mb-1">
                <div className="relative flex items-center justify-center">
                  <svg width="72" height="72" className="rotate-[-90deg]">
                    <circle cx="36" cy="36" r="30" fill="none" stroke="rgba(217, 70, 239, 0.15)" strokeWidth="4" />
                    <circle
          ...
          {currentTrack?.previewUrl ? (
            <SongPlayer previewUrl={currentTrack.previewUrl} albumArt={currentTrack.albumArt} />
          ) : (
  ...
  return (
    <div ref={centerRef} style={{ width: '100%', display: 'flex', justifyContent: 'center', opacity: 0 }}>
      {renderContent()}
    </div>
  );
}
